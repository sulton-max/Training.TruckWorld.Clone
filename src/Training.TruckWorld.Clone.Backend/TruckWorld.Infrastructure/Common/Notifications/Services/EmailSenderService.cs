...
{
    private readonly IEmailSenderBroker _mailSenderBroker;
    private readonly IValidator<EmailMessage> _emailMessageValidator;

    public EmailSenderService(IEmailSenderBroker emailSenderBrokers, IValidator<EmailMessage> validator)
    {
    ...
    public async ValueTask<bool> SendAsync(EmailMessage emailMessage, CancellationToken cancellationToken)
    {
        ...
        var sendNotificaitonTask = () => _mailSenderBroker.SendAsync(emailMessage, cancellationToken);
